...
public class CustomAudioDevice implements AudioDevice {
    ...
    // Required for implementing effects
    public int audioSessionId;
    public int recordAudioSessionId;
    ...
    private final Runnable microphoneCapturerRunnable = () -> {
        ...
        try {
            ...
            return; // Exit the Runnable if startRecording fails
        }

        while (true) {
            int bytesRead = audioRecord.read(micWriteBuffer, micWriteBuffer.capacity());
            if (bytesRead > 0) { // Changed condition to check for positive bytes read
    ...
    public boolean onInitCapturer() {
        ...
        recordAudioSessionId = audioRecord.getAudioSessionId();
    ...
    public boolean onInitRenderer() {
        ...
        Log.i(TAG, "AudioTrack initialized with session ID: " + audioSessionId);
        keepAliveRendererRunnable = true;

        // Ensure AudioTrack is properly initialized
